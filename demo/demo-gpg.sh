...
GPGHOMEDIR1=/tmp/gpgs/1
GPGHOMEDIR2=/tmp/gpgs/2

rm -rf /tmp/gpgs

mkdir -p $GPGHOMEDIR1
chmod 700 $GPGHOMEDIR1
mkdir -p $GPGHOMEDIR2
chmod 700 $GPGHOMEDIR2

gpg --homedir $GPGHOMEDIR1 --import $CONF/key1
gpg --homedir $GPGHOMEDIR2 --import $CONF/key2

exe export PANORAMIX_CONFIG=$CONF/user1_settings
...
exe $CMD peer info --peer-id 13C18335A029BEC5

ENDPOINT1="ONION_PEER1"

echo
read -p "Create endpoint $ENDPOINT1: " Y

with_self_consensus "endpoint create --endpoint-id ${ENDPOINT1} --peer-id 13C18335A029BEC5 --endpoint-type ONION --size-min 3 --size-max 10"
...
echo
exe $CMD endpoint list --peer-id 13C18335A029BEC5

echo
exe export PANORAMIX_CONFIG=$CONF/user2_settings
...
echo
exe $CMD peer import --peer-id 13C18335A029BEC5

echo
read -p "Send first message: " Y

echo
exe $CMD message send --recipient 13C18335A029BEC5 --endpoint-id ${ENDPOINT1} --data "first text"
...
echo
exe $CMD message send --recipient 13C18335A029BEC5 --endpoint-id ${ENDPOINT1} --data "second text"

echo
exe $CMD message send --recipient 13C18335A029BEC5 --endpoint-id ${ENDPOINT1} --data "third text"
...
exe $CMD inbox process --peer-id 13C18335A029BEC5 --endpoint-id ${ENDPOINT1} --process-log-file "${PROCESS_LOG}" --upload
